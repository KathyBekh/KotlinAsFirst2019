...
fun countSubstrings(inputName: String, substrings: List<String>): Map<String, Int> {
    ...
    for (i in text.lines()) {
        val line = i.toLowerCase()
        for (e in substrings) {
            val element = e.toLowerCase()
            val occurrancy = line.split(element).size - 1
            numberOfOccurrences[e] = numberOfOccurrences.getValue(e) + occurrancy
...
fun sibilants(inputName: String, outputName: String) {
    val hissingLetters = listOf('ж', 'ч', 'ш', 'щ')
    val replaceableLetters = mapOf('ы' to "и", 'я' to "а", 'ю' to "у")
    File(outputName).bufferedWriter().use {
        for (line in File(inputName).readLines()) {
            it.write(line[0].toString())
            for (i in 1 until line.length)
                if (line[i - 1].toLowerCase() in hissingLetters && line[i].toLowerCase() in replaceableLetters)
                    it.write(
                        replaceableLetters.getOrDefault(
                            line[i],
                            replaceableLetters.getOrDefault(line[i].toLowerCase(), "").toUpperCase()
                        )
                    )
...
fun chooseLongestChaoticWord(inputName: String, outputName: String) {
    ...
    for (line in text) {
        val includeSymbols = line.toLowerCase().toSet()
        if (line.length == includeSymbols.size && includeSymbols.size >= max) {
            max = includeSymbols.size
            theLongestWords.add(line)
        }
    }
    File(outputName).bufferedWriter().use { it.write(theLongestWords.joinToString(", ")) }
}
